...
public class PostController {
    ...
    /**
     * 게시물 등록 @Headers Authorization
     *
     * @body PostCreateDto.Request
     */
    @PostMapping
    public ResponseEntity<Void> create(
            @Auth AuthUser user, @RequestBody @Valid PostCreateDto.Request request) {
        postService.create(
                user.getId(), request.getTitle(), request.getContent(), request.getCelsius());
     ...
     */
    @GetMapping
    public ResponseEntity<Page<GetPostsDto.Response>> getPosts(
            @RequestParam(defaultValue = "1") int page,
            @RequestParam(defaultValue = "10") int size) {
        return ResponseEntity.ok(postService.getPosts(page - 1, size));
    }

    /**
     * 뉴스피드 조회
     *
     * @return Page<GetPostsDto.Response> @Headers Authorization
     * @query page, size
     */
    @GetMapping("/followers")
    public ResponseEntity<Page<GetPostsDto.Response>> getFollowerPosts(
            @Auth AuthUser user,
            @RequestParam(defaultValue = "1") int page,
            @RequestParam(defaultValue = "10") int size) {
        return ResponseEntity.ok(postService.getFollowerPosts(user.getId(), page - 1, size));
    }
    ...
    public ResponseEntity<GetPostDto.Response> getPost(@PathVariable Long id) {
        ...
        return ResponseEntity.ok(new GetPostDto.Response(post));
    }
     ...
     *
     * @param id
     * @return PostUpdateDto.Response @Headers Authorization
     * @body PostUpdateDto.Request
     */
    @PatchMapping("/{id}")
    public ResponseEntity<PostUpdateDto.Response> update(
            @Auth AuthUser user,
     ...
     * 게시물 삭제
     *
     * @param id @Headers Authorization
     */
    @DeleteMapping("/{id}")
